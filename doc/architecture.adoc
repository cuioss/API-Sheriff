...
| The gateway is the OIDC confidential client. The tokens (access token, refresh token, minimal
  metadata) travel only inside an AES-256-GCM-encrypted `HttpOnly` cookie, decrypted per request
  before upstream injection.
|===
