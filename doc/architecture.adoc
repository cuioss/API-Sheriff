...
The management port is served over *HTTPS*. Quarkus' management interface has exactly one port --
its configuration declares no `ssl-port` and no `insecure-requests` key, unlike `quarkus.http` --
so supplying `quarkus.management.ssl.certificate.*` converts port `9000` itself to HTTPS rather
than adding a second listener. The consequence is that no plain-HTTP management surface remains:
every scraper and probe must speak TLS.
...
(`/q/health/live`, `/q/health/ready`) on the same management port, and therefore over the same
HTTPS single-port surface described above. Readiness reflects the loaded-and-validated
configuration and, for the BFF variants, the ability to reach the configured issuer at startup.
The management port is not exposed publicly, so health and metrics endpoints carry no
authentication of their own -- TLS now additionally protects them in transit, but it is transport
protection, not authentication. Data-plane routes such as a public `/health` proxy are ordinary
