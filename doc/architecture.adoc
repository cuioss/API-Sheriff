...
`security_defaults.allow_get_with_content_length_body`: the body-present and
declared-`Content-Length` legs, and only for `GET`. It exists because some upstream APIs genuinely
take a body on `GET` (Elasticsearch `_search`). The boundary is the architectural point --
