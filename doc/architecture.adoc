...
*What `profile: none` costs, exactly.* `none` is a *partial* disable
...
profile switch. A route whose effective profile resolves to `none` is additionally *refused at boot*
...
| url-parameter *name* and *value* validation | post-route | *yes* -- off under `none`
| Divergent path/header pipeline re-run | post-route | *yes* -- off under `none`
|===
...
*Profiles.* The mode set is `strict` / `lenient` / `none`, on the gateway-wide `security_defaults`
...
configuration validator owns the *posture* rule -- the fail-closed refusal of an effective `none`
on an effectively-authenticated or `type: bff` route (see *What `profile: none` costs, exactly.*
above). `none` contributes no limits policy of its own, so a `none` route takes its limits from the
nearest non-`none` profile in its fallback chain (landing on `strict` when the whole chain is
`none`), which is what keeps its retained body cap concrete. See
link:adr/0024-Inbound_filter_modes_none_is_a_partial_disable_retaining_the_body_cap_and_path_allowlist_and_is_refused_on_authenticated_and_BFF_routes.adoc[ADR-0024].
...
* A shared, immutable `SecurityConfiguration` supplies the tunable limits. The gateway sources it
  ...
  `security_filter` limit keys -- never from bare builder defaults. `none` has no preset of its own
  and inherits the nearest non-`none` one (see *Profiles* above).
