...
resolves to `strict`, the fail-closed choice. The value *range* is enforced by the bundled JSON
Schema, not by the configuration validator. `none` contributes no limits policy of its own, so a
`none` route takes its limits from the nearest non-`none` profile in its fallback chain (landing on
`strict` when the whole chain is `none`), which is what keeps its retained body cap concrete. See
link:adr/0024-Inbound_filter_modes_none_is_a_partial_disable_retaining_the_body_cap_and_path_allowlist_and_is_refused_on_authenticated_and_BFF_routes.adoc[ADR-0024].
