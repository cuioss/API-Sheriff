...
* *Reading a body the gateway itself consumes* (the `response_mode=form_post` callback and
  back-channel logout): read it on the event loop *before* the virtual-thread hop, then dispatch
  with the buffered bytes -- see `GatewayEdgeRoute#readReservedBodyThenDispatch`. A proxied body
  is never buffered; it streams on the client's event loop (ADR-0008).
...
loop. The failure signature is a body-read timeout surfacing as a downstream 400 (a
form_post callback rejected "missing state"). This is a threading defect, not a test flake:
reproduce it locally by pinning the app container's CPU (`docker update --cpus 1 <container>`)
and running the integration suite against the running stack.
====
