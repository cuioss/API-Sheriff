...
* *The security-events meter is not yet bridged from the `SecurityEventCounter`.* `GatewayEdgeRoute`
  now records the request, per-route duration, error, and upstream-duration meters end-to-end on the
  hot path -- `sheriff_requests_total{route,method,status_family}`, `sheriff_request_duration_seconds{route}`,
  `sheriff_errors_total{route,category}`, and `sheriff_upstream_duration_seconds{route}` -- with the
  bounded `route` label (config-fixed id, or `"<no-route>"` when unmatched) and the `status_family`
  bucket keeping label cardinality fixed. `MetricsIT` scrapes `/q/metrics` and asserts those meters
  appear with their bounded labels after proxied traffic. The remaining
  `sheriff_security_events_total{failure_type}` meter is still unbridged: the in-process cui-http
  `SecurityEventCounter` accumulates the per-`UrlSecurityFailureType` counts, but no Micrometer binder
  yet exposes them, so that single meter is not emitted end-to-end. Readiness (`GatewayReadinessCheck`)
  *is* wired: `/q/health/ready` reflects the loaded configuration and the gateway validator's JWKS status.
