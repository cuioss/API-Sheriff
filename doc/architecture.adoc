...
* *Metrics emission is not yet wired into the hot path.* The Micrometer adapter `SheriffMetrics`
  exists and registers exactly the meter names named in link:#_metrics[Metrics] (`sheriff_requests_total`,
  `sheriff_request_duration_seconds`, `sheriff_errors_total`, `sheriff_security_events_total`,
  `sheriff_upstream_duration_seconds`), and its names are asserted by a unit test. The per-request
  edge stages, however, still record only into the in-process `GatewayEventCounter` /
  `SecurityEventCounter`; the bridge that reads those counts (and the per-route request/upstream
  timers) into `SheriffMetrics` is not yet called from `GatewayEdgeRoute`, so the `sheriff_*` meters
  are not yet emitted end-to-end. The management `/q/metrics` endpoint is live and exposes the
  Micrometer JVM/HTTP baseline. Readiness (`GatewayReadinessCheck`) *is* wired: `/q/health/ready`
  reflects the loaded configuration and the gateway validator's JWKS status.
