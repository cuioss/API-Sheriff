...
construct, and its `issuer_reachability` datum is reported `unverified` rather than asserted --
a BFF `mode: server` deployment reaches its issuer lazily on the first login, and that does not
gate boot readiness.

That reading is why the DOWN branch is, in the shipped topology, effectively unreachable rather than
a routine outcome. `TokenValidatorProducer.onStartup` performs the same eager `Instance.get()` the
probe performs, so a validator that cannot be constructed aborts the boot before the management
