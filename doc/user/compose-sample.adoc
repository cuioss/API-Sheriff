...
. Switch `docker/sheriff-config/endpoints/demo-api.yaml` to `auth: require: bearer`. You will
  ...
  parameter crosses except the gateway-owned never-forward set (`Cookie`, `Authorization`, the
  hop-by-hop and framing names), and the HTTP mechanics -- `Accept`, `Content-Type`, `Range` and
  the conditional validators -- cross without being listed. Add a `forward` block only to
