...
- id: orders-grpc
  protocol: grpc
  match: { path_prefix: /orders.OrderService, host: api.example.com }
  auth: { require: bearer }
  upstream: { host: orders-svc, port: 50051, path: "" }
----

A gRPC route carries no extra per-route fields beyond the standard `match`, `auth`, and
`upstream` keys -- the forced-HTTP/2 upstream dial and the trailer relay are automatic. Point
`upstream` at the gRPC service's `host:port`; the gateway holds a forced-h2 client for that
target distinct from any HTTP/1.1 route to the same `host:port`.
