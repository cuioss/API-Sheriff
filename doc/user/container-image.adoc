...
matching `QUARKUS_*_ROOT_PATH` environment variables are silently inert against it. A deployment that
needs a different prefix builds its own image through the Maven seam, or inherits the published build
parent; link:context-path.adoc[The Context Path] documents both routes and the reason there is no
runtime one.
