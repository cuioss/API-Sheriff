...
signal: readiness is a `GET` against the readiness endpoint on 9000, beneath the management context
path the image was built with, and a healthy container has not told you the gateway is ready to
serve. What the signal does and does not prove, and how to read the probe's own
...
*The context paths are properties of the image, not of the run.* Both the data-plane prefix and the
management prefix are fixed at build time -- the published image ships the defaults, and the
matching `QUARKUS_*_ROOT_PATH` environment variables are routing-inert against it: they move nothing,
though a divergent value may draw a startup build-time-mismatch warning naming the key. A deployment
that needs a different prefix builds its own image through the Maven seam, or inherits the published
build parent; link:context-path.adoc[The Context Path] documents both routes and the reason there is
no runtime one.
...
moving the probe, and the container will never report healthy however well it is serving. The only
...
before the image is pushed to GHCR if Trivy reports a finding at *HIGH or CRITICAL* on that same
image. The gate is the `Scan the tested image (gating, HIGH and CRITICAL)` step of
link:../../.github/workflows/release.yml[`.github/workflows/release.yml`], which runs ahead of the
registry login. It cannot hold back the Maven artifacts, which the release has already published by
then -- see link:../development/release-process.adoc#release-not-atomic[The release is NOT atomic].
...
// The explicit ID pins the auto-generated one, so existing deep links to this section keep working
// and the internal cross-reference above resolves without relying on Asciidoctor's ID derivation.
[#_verify_the_signature_before_deploying]
...
distribute. Signing is the `Sign the published digest (keyless)` step of
...
The signing identity is the release workflow's ref, and the two releases whose certificates were
inspected, 0.1.0 and 0.1.1, signed under the same one -- `@refs/heads/main`. That is not inferred
from the trigger configuration; it is read off the certificates themselves. Those two digests carry
an identical `job_workflow_ref` SAN even though they were produced by *different* triggers:
...
does not verify* -- that is the `Verify the signature under the published identity` step of
link:../../.github/workflows/release.yml[`release.yml`]. The values above are not a transcription
of what the workflow is believed to do -- they are what it proves before calling a release
complete.
