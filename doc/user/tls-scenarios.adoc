...
| Reach the BFF's identity provider (discovery, code exchange, refresh) through a private CA or a
...
identity provider -- discovery, the code exchange and refresh -- is a separate leg with a separate key,
...
exchange and refresh. That leg carries the client secret and the authorization codes, and it has two
...
* `egress_tls.oidc_tls_profile` -- the provider presents a certificate from a private CA. The named
  profile *replaces* the JVM default trust store on discovery, the code exchange and refresh: public
...
* *Tokens validate but login fails.* The JWKS leg and this leg are configured separately: a
  `jwks.tls_profile` or `jwks_verify_hostname` does not reach discovery, the code exchange or refresh.
