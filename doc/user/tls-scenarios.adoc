...
* *The management certificate pair is not supplied.* The management listener starts plain, and
  `ManagementPlainHttpAudit` reports it with `WARN ApiSheriff-115`. HTTPS probes against the port fail.
...
* *A default `+quarkus.tls.key-store.*+` bucket declared anywhere.* The management interface falls
  back to that bucket when no configuration name is selected, so it can be switched to HTTPS through a
  path no management setting mentions. `ManagementPlainHttpAudit` reads that bucket as the runtime
  does, so `ApiSheriff-115` stays truthful, but the posture is no longer the one your management
  settings describe.
...
exchange and refresh. Those are two independent legs with two different trust levers, and a private-CA
...
* When the provider sits on a private address, its host named in `allowed_egress_hosts`.
* For the BFF back-channel, a store the JVM default trust manager can use. It *replaces* the platform
  bundle, so build it to keep the public roots you still need -- see
...
*Deployment, BFF back-channel leg* -- `token-sheriff-client` exposes no per-client trust seam
(https://github.com/cuioss/token-sheriff/issues/597[token-sheriff-client#597]), so the JVM default
trust store is the only lever. On the native image it has to arrive as command-line `-D` arguments.
...
* *No profile, and a JVM default trust store that does not hold the provider's CA.* The JWKS loader
  ...
  looks like a token problem. `TokenValidatorProducer` assembles the validator at startup, so a profile
  that cannot be resolved surfaces at boot rather than on the first request.
...
asset origin or the BFF back-channel does too. Do not name a profile holding only the corporate CA
