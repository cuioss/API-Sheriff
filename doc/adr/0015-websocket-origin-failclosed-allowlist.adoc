...
* *A bearer WebSocket route has no existing origin allowlist to reuse.* The BFF variants already
  carry `session.csrf.trusted_origins`, and the *intended* posture for a `require: session` WS route
  is to reuse it -- the WS Origin check and the stage-0 CORS/CSRF posture are the same trust decision
  there. *That reuse is not wired at 0.1.0* -- see the WARNING in the Decision section below; it is
  recorded here as the force that shaped the field's scope, not as shipped behaviour. A *bearer* WS
  route has no such set to reuse under either reading.
...
Introduce an `allowed_origins` field inside the per-route `websocket` block,
`security_headers`-adjacent but scoped *per route* on bearer WebSocket routes, with *fail-closed
exact-match* semantics defined *doc-first* in
...
* *Fail-closed, no "any origin" default.* The handshake `Origin` must exactly match a listed
  ...
  `websocket.allowed_origins` on a bearer WebSocket route *fails the boot* -- there is
  deliberately no accept-any default.
* *Bearer only.* The field is validated -- and its fail-closed boot check enforced -- only on a
  `protocol: websocket` route whose effective auth is `require: bearer`. A socket is never
  authenticated on the ambient session cookie alone.
+
[WARNING]
====
*The intended `require: session` posture -- reuse `session.csrf.trusted_origins`, keeping the WS
Origin check and the CORS/CSRF posture one decision -- is NOT implemented at 0.1.0.* Nothing wires
`session.csrf.trusted_origins` into the route table's WebSocket allowlist: a session WS route that
declares no `websocket.allowed_origins` reaches boot with an *empty* allowlist, and an empty
allowlist means *no enforcement* -- the upgrade proceeds without an Origin check. Only the bearer
leg fails the boot on an absent allowlist.

*Until the reuse lands, a session WebSocket route must declare `websocket.allowed_origins`
explicitly.* An explicitly declared allowlist is enforced on any route, so the control is
reachable; it is the automatic inheritance that is missing, not the check itself. This states the
same position as link:../security-threat-model.adoc#gw-09[threat model GW-09], whose status is
*GAP for session-authed routes*. The two documents agree, and this bullet must not be read as
claiming coverage the code does not deliver.
====
...
* *CSWSH stays open for `require: session` WebSocket routes that do not declare an allowlist
  themselves* -- the fail-closed boot check covers the bearer leg only, so the session leg carries
  no floor. This is the residual the Decision's warning above records and that
  link:../security-threat-model.adoc#gw-09[GW-09] tracks as a gap to close.
...
* link:../configuration.adoc#_allowed_origins[Configuration -- `websocket.allowed_origins`] -- the
