...
host and no port number is restated in the script. Adding, removing, or renumbering
an instance requires no script edit.
...
at it. A single retry-budget constant is declared once and read by every site that
needs it -- the loop bound, the last-attempt comparison, the progress message -- so
the number cannot drift between spellings.
...
* Where two bring-up scripts derive from the same model independently, the
  derivation logic is duplicated. Duplicated derivation is materially safer than a
  duplicated literal -- both stay correct when the model changes -- but it remains
  duplication and should be consolidated when a shared library exists.
