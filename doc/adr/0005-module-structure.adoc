...
[IMPORTANT]
====
*Pending supersession.* This ADR is expected to be *reversed* by `api-sheriff-next`, on the
operator decision _"Revert the ADR. If there is something available (quarkus) use that"_. That
...
| `deployment` | Deployment-shaped `docker-compose` sample. Added after this ADR was accepted. Carries no Java and builds no artifact.
|===
...
=== What the reversal measures against
