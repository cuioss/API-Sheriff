...
The early interim proxy exhibited the hazard directly: it read the upstream response with
...
* *Streaming-specific responses pass through untouched.* SSE (`text/event-stream`), chunked
  ...
  a WebSocket upgrade becomes a byte relay after the authenticated handshake. None of
...
* The request pipeline's dispatch and response stages replaced the interim `ofByteArray()`
  buffering proxy with the streaming pipe, and the body cap is a streaming counter.
