...
* *Namespace coverage at boot (the mirror of the squatter rule).* The two directions above are
  *declaration*-scoped: they judge a route by where its own match key sits, so a route sitting
  ...
  one address, which is the squatter rule's case), and `protocol: grpc` routes are exempt for the
  same geometric reason as below.
* *gRPC routes are exempt from the namespace-containment geometry.* A `protocol: grpc`
  ...
  All three containment checks -- including the coverage mirror above -- are therefore
  skipped for `protocol: grpc` routes. Only the
