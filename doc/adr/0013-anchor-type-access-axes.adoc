...
* `type` (`proxy` \| `bff` \| `asset`) -- what the anchor is. `proxy` is a plain reverse-proxy
  ...
  (link:0014-asset-serving-terminal-action.adoc[ADR-0014]). This value space is entirely
  separate from the TLS-L4 `tls.passthrough_sni` surface, despite the adjacent vocabulary.
...
fail-closed boot matrix: `access:
...
* `passthrough` is not an anchor `type` value; the TLS `tls.passthrough_sni` surface is a
  separate concern, so the two cannot be confused.
