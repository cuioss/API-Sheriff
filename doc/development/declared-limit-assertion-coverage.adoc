...
What makes this overlay worth naming is that it is the fixture the standing remainder's
session-refresh leeway item needs. The shared realm mints a 900-second access token, which puts the
...
| `oidc.session.refresh.leeway_seconds: 30`
  `gateway.yaml` ×8 -- base:293, cookie:174, egress-verify-off:143, egress-verify-on:143,
  mtls:154, passthrough-empty:309, refresh:279, ws-admission:133
...
| `BffTokenRefreshIT` on the dedicated `api-sheriff-refresh` instance -- the only place the boundary
  ...
  `lifespan - leeway` = 15s, and the row's matched control pair straddles exactly that edge:
  `currentOutcomeReusesTheMediatedToken` calls *inside* the window and asserts a byte-identical
  bearer (CURRENT -- nothing rotated), while `refreshedOutcomeRotatesTheMediatedToken` calls *after*
  it opens and asserts a different bearer (REFRESHED). Move the declared 30 either way and one of
  the two goes red, which is what makes this the boundary rather than a smoke test.
...
| COVERED
| The nine cookie-mode ITs (`BffCookieSessionIT` ×6, `BffCookieStatelessnessIT` ×3) are the
  assertion, by construction rather than by naming the number: this instance seals access + id +
  refresh into 5123 bytes, so at the 4096 default the codec refuses the seal (`ApiSheriff-114`) and
  every one of the nine fails `302` vs `500` at login. Delete the key and all nine go red -- the
  deleted-key test the note's own methodology asks for. The negative control is the *other* seven
  instances, which declare no budget and keep the 4096 default without ever approaching it.
