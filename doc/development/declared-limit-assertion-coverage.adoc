...
removed, so every key the base declares it also declares, at the base's own relative order. Citing
it per row would add a fifth line number to twelve rows to restate one fact that is better stated
once: *derive it from the base citation.* The single consequence of the derivation is the one key it
does **not** carry -- `tls.passthrough_sni`, whose absence is precisely the property that instance
exists to provide (see the descriptor's own header comment).
