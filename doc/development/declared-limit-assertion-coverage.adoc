...
The declared surface is *fifteen files* under `integration-tests/src/main/docker/`.
...
Over all fifteen files it returns hits in exactly twelve of them:
...
none. That complement is the other half of the same claim, so asserting the twelve exactly asserts
both halves at once.
