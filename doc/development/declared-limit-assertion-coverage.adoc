...
The declared surface is *sixteen files* under `integration-tests/src/main/docker/`.
...
Only the base directory carries `endpoints/` and `topology.properties`; the six overlay
directories replace `gateway.yaml` and nothing else, which is why the endpoints tree resolves
identically for all nine booted gateway instances.
...
Over all sixteen files it returns hits in exactly twelve of them:
...
claim, so asserting the twelve exactly asserts both halves at once.
...
runtime -- the inventory table, the sweep pattern and the twelve limit-bearing paths -- and uses them
...
twelve. If this note cannot be located, or a marked block is missing or empty, the guard fails loudly
...
. *The enumerated surface is stated explicitly* (sixteen files, named) *and machine-checked against
  this note*. `DescriptorInventoryWiringTest` parses the marked blocks in "The Enumerated Surface"
  -- the inventory table, the sweep pattern and the twelve limit-bearing paths -- and uses them as its
  ...
  non-limit-bearing file limit-bearing* -- when it moves a file across the twelve/four partition.
...
When you add a limit-shaped key to any of the sixteen files, add its row here in the same change --
