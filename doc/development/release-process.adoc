...
* *Trivy gates the release at HIGH and CRITICAL.* The gate is the
  `Scan the tested image (gating, HIGH and CRITICAL)` step of
  link:../../.github/workflows/release.yml[`release.yml`]. A finding at either severity fails the
  job, and the step runs before the registry login, so nothing has been pushed to GHCR when it fires
  (the Maven artifacts are already out -- see <<release-not-atomic>>). The threshold is
...
* *A Syft SBOM is produced for the same image* the gate scanned, and retained as an artifact of the
  release run.
* *The image is Cosign-signed keylessly, by digest*, under the workflow's GitHub OIDC identity, in
  the `Sign the published digest (keyless)` step. No key material and no new secret is involved. A
  tag is a mutable pointer, so signing one would sign nothing durable. The following
  `Verify the signature under the published identity` step then runs the operator verification
  command from link:../user/container-image.adoc[the container image guide] against that digest and
  fails the release if the signature does not verify under the published identity.
...
*Provenance of the discriminator.* link:../../.github/workflows/release.yml[`release.yml`] declares
`released-version` the run/skip discriminator for the downstream jobs, in the comment block and the
`if:` of its `publish-image` job: an empty value skips them. What an empty value proves is exactly that *no release was produced* -- not that
