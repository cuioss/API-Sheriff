...
=== This contract covers the integration-tests bring-up only

Everything above describes `integration-tests/scripts/start-integration-container.sh`. The parallel
developer bring-up, `demo-client/scripts/start-dev-environment.sh`, still gates its gateway wait on
`/q/health/live` with an unmeasured 30-attempt budget. The two scripts therefore *diverge* on the
readiness contract from this change onward.

That is a stated decision, not an oversight: the change was deliberately scoped to the
integration-tests bring-up, whose suite is the thing a half-initialised instance actually breaks.
The demo bring-up is driven interactively by a developer who sees a failing first request, so the
same exposure carries a far smaller cost there. Aligning it is a follow-up; until then, do not read
this section as describing the demo environment.
