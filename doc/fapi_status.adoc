= API Sheriff -- FAPI 2.0 Conformance Status (0.2.3 Alpha)
:toc: left
:toclevels: 3
:sectnums:
:sectnumlevels: 2
...
*FAPI 2.0 Security Profile (Final)* as of the 0.2.3 alpha release. It exists because the profile is
...
[IMPORTANT]
====
*Verdict: API Sheriff 0.2.3 is NOT FAPI 2.0 conformant.* Three of the profile's mandatory client
...
| `BffRuntimeProducer` constructs `AuthorizationCodeFlow` with a `null` `senderConstraint`, and
  `RefreshFlow` from the configuration, token-endpoint client, validation bridge and client
  authentication alone, passing no sender constraint. Tokens are plain bearer tokens carrying no
  `cnf` claim.
