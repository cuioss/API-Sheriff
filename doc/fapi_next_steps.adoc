...
The route from the 0.2.3 alpha position recorded in link:fapi_status.adoc[FAPI 2.0 Conformance
...
and `RefreshFlow` (the gateway builds it today from the configuration, token-endpoint client,
validation bridge and client authentication alone, so a sender-constraint-carrying construction
replaces that one). *It must be the same instance* -- a
