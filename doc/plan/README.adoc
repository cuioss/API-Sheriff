...
* *Dependency approvals*: every new Maven dependency a plan needs is flagged inside the plan
  ...
  (CLAUDE.md rule). TokenSheriff artifacts use the released line, never snapshots -- managed by
  `token-sheriff-bom`, whose version is the `version.token-sheriff` property in the root
  `pom.xml` (currently `0.9.6`; read the property, not this line). The whole stack
  (validation + client modules) is already in the build, so a plan that consumes it wires an
  existing dependency rather than adding one -- and the approval gate applies only to what a
  plan genuinely introduces.
