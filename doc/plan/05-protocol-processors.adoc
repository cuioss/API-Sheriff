...
* *Benchmark tooling -- Clarification 1.* The `ghz`/`k6` split in the Work Breakdown is
  ...
  introduced.
* *Config model is final -- no config-model change in this plan.* The per-route config surface
...
. *Remove the orphaned JAX-RS `GatewayExceptionMapper`.* `edge.GatewayEdgeRoute.renderProblem(...)`
  ...
  (`@Provider ExceptionMapper<GatewayException>`) is never reached by the data plane -- it is dead
  and is removed (subject to a live-wiring stop-guard).
. *Investigate "verify reports green while tests errored".* A gate defect where the build wrapper
  reported success despite errored tests; investigated and fixed so a genuine test error fails the
  build.
--
