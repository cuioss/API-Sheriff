...
[NOTE]
====
*Delivered -- read as a historical record, not as the current specification.* Per
link:README.adoc[the plan index], the design documents govern and the plans only sequence the
work; this one records what was decided and built at Plan 04 time.

One part of it has since been superseded. *Stage 5's forward policy* is described below as an
allowlist ("`headers_allow` / `query_allow` filtering", the two conditional validators joining
the effective allowlist).
That is no longer how the stage works: the forward block now resolves one of *three modes* per
dimension -- positive-list, negative-list, or forward-all -- inside a route the request is
already authorised for, bounded by gateway-owned header sets no route configuration can
override, and the conditional tier carries five validators rather than two.

*One rule below is retained, not superseded.* "Inbound `Authorization` stripped unless explicitly
allow-listed" still holds exactly as written: `Authorization` is the single re-admittable member
of the gateway-owned never-forward set, and a route's positive-list naming it remains the only
thing that re-admits it -- so a negative-list or forward-all route withholds it, and on a
`require: session` route the mediated bearer overwrites it regardless. `Cookie` is the contrasting
case, absolute against client input under every mode. The current
specification is
...
with the operator-facing reference in link:../configuration.adoc#_forward[`configuration.adoc`].

Deny-by-default is unchanged and is *not* what moved: it lives at the URL layer (stage 2, route
selection), exactly as this document's stages 1-2 and its GW-01 item describe.
====
