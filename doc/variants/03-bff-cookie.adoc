...
| Size
| The cookie must stay within browser limits (~4 KB). If the token set exceeds the budget,
  the refresh and/or ID token may be split into their own cookies; oversized IdPs are a
...
restart and the key cannot be shared with another replica. `previous_key` is optional and
decrypt-only, and requires an `encryption_key` to roll onto. See
