...
*Status:* PARTIAL/COVERED. Streaming + the `max_body_bytes` counter is ADR-0006 + pipeline
stage 6; edge limits, idle timeout, admission cap and graceful drain are delivered as the
edge-hardening bounds; YAML limits are applied at config load.
...
proven by the traversal/encoding corpus.
