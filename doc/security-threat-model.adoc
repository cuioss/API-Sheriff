...
a real handshake fail on one leg and succeed on the other. The *JWKS* half is *not delivered and not
proven*: `egress_tls.jwks_verify_hostname` is declared in the schema and bound on the configuration
model, and *no production code reads it*, so it is a reserved key rather than an available control
and nothing asserts its behaviour; PLAN-04 owns both the reader and its behaviour test. The
*fail-closed-on-empty-SNI* rule and *full-ClientHello reassembly before routing* remain unpinned (see
Gaps), which is why this entry stays PARTIAL.
...
trusted anchor is *still refused*, so the two mechanisms are proven uncoupled. No assertion covers
`jwks_verify_hostname`, and none may be claimed for it while nothing reads it.
...
| <<gw-06>> | Fail-closed SNI/mTLS; verify upstream certs by default; test behaviour not flag | ADR-0017 (front listener) + ADR-0040 (`egress_tls`: upstream verify behaviour-tested, trust profile fail-closed; `jwks_verify_hostname` reserved, unread) | PARTIAL
...
. *GW-06 -- fail-closed SNI + behaviour tests.* Make explicit: empty/fragmented SNI
  ...
  (link:adr/0040-Upstream_hostname_verification_is_a_global_egress-TLS_knob_bound_at_client_construction.adoc[ADR-0040]).
  What remains under this heading is the mTLS-require boolean, the two SNI rules above, and
  `egress_tls.jwks_verify_hostname`, which has no production reader yet and therefore no behaviour
  to test; PLAN-04 owns both.
