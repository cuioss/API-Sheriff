...
* *The request-direction never-forward set applies under every forward mode* -- including the
  ...
  route declaring a positive-list naming it); `Cookie` is absolute. No route configuration
  re-admits the hop-by-hop, framing or gateway-owned names.
...
Forward-all removes that incidental protection, so the protection is now stated: those spellings
are members of the request-direction never-forward set in their own right, withheld under every
mode without an operator entry. They sit *there* rather than beside the canonical names because
the gateway does not *regenerate* them -- the canonical names are what it emits, the variants are
...
*The equivalence class beyond the canonical names is covered too, and by a different mechanism.*
The regeneration skip is keyed on the canonical spellings (`x-forwarded-for`, `-host`, `-proto`,
`-port`, `-prefix`, `forwarded`), matched case-insensitively -- that is what the gateway *emits*.
The spellings it does not emit but must still never *relay* -- the underscore forms
(`X_Forwarded_For` and its four siblings) and the vendor aliases (`X-Real-IP`, `X-Client-IP`,
`True-Client-IP`, `CF-Connecting-IP`) -- are members of the request-direction never-forward set
instead, so they are withheld under every mode with no operator entry. Splitting the class across
the two sets by *which question each name answers* is deliberate: filing a name the gateway does
not regenerate beside the ones it does would assert a relationship that is not there, and would
widen the untrusted-peer resolver mask with names the resolver never queries. `Forwarded`
contributes no separate variant -- it carries no hyphen to fold, so its underscore spelling is
the canonical name already held by the regeneration skip. The Traefik fix-chain in the evidence
above is a chain of *incomplete* equivalence-class handling; closing the class in the gateway's
own sets, rather than per route, is what keeps this from becoming another link in it.
...
each leg carrying a matched positive control; an operator's `set_headers` still places such a
name deliberately; a `0.0.0.0/1`+`128.0.0.0/1` config is rejected at boot.
