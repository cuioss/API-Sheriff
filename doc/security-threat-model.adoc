...
taking effect silently. A *fourth* terminated-egress leg is outside that block and is recorded below
rather than folded into the claim. The
...
together with an issuer's per-issuer `jwks.tls_profile`, which supplies an SSL context the relaxation
cannot act on -- is *refused at boot* with a configuration error naming both keys and the issuer,
rather than silently ignoring one of them. The
...
production unremarked; token-sheriff's SSRF egress guard still constrains which hosts the JWKS fetch
may reach at all, so redirecting it is not merely a DNS answer away; and the scope claim is proven by
the untrusted-chain dial above rather than asserted. The mitigation for an operator whose IdP is
behind an internal CA is `jwks.tls_profile`, which is a different key answering the chain-trust
question -- and, on this leg, the two are *mutually exclusive per issuer* and the combination is
refused at boot, so an operator cannot reach for both at once and end up with neither.
...
| <<gw-06>> | Fail-closed SNI/mTLS; verify upstream certs by default; test behaviour not flag | ADR-0017 (front listener) + ADR-0040 (`egress_tls`: upstream verify behaviour-tested, trust profile fail-closed) + ADR-0041 (`jwks_verify_hostname` bound and behaviour-tested; `jwks.tls_profile` collision refused at boot) | PARTIAL
...
. *GW-06 -- fail-closed SNI + behaviour tests.* Make explicit: empty/fragmented SNI
  ...
  proven by the same three-legged shape at unit level, with its collision against a per-issuer
  `jwks.tls_profile` refused at boot
  (link:adr/0041-The_JWKS_hostname-verification_knob_is_the_librarys_own_passthrough_and_its_sslContext_collision_is_refused_at_boot.adoc[ADR-0041]).
