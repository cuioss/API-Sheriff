...
*Status:* PARTIAL. Header CR/LF/NUL is cui-http (COVERED); zero-trust regeneration is pipeline
...
configuration validator (CIDR parsing, split-range trust-all rejection). The
name-equivalence-class stripping is the one nuance still to make explicit in the test set.
...
| <<gw-03>> | Header CR/LF reject; hop-by-hop cannot strip framing/trust | cui-http (CR/LF) + gateway (hop-by-hop) | PARTIAL
| <<gw-04>> | TCP-peer forwarded gate; regenerate; CIDR validation | cui-http resolver + ADR-0003 + gateway (peer gate, trust-all rejection) | COVERED
