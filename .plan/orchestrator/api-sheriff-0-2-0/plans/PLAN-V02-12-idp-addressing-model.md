...
All re-checkable claims hold (single `issuer` field, the sibling issuer/jwks keys, `frontendUrl: https://keycloak:8443` ×3, the playwright host-resolver hack, the directly published Keycloak ports, no `/auth` literal in production code, the six `/auth` reserved paths). Claim 11 stays refuted-and-absorbed. Keycloak is still `26.5.7`. D1–D3 are open.
...
- OBSERVED: the topology SVG omits `api-sheriff-plain-mgmt` in both drawing and `<desc>`, disclosed
  ...
  - verdict: contradicted | checked_at: 05f6ee3ebb5ae32fb75082b660e6abdb7617edb6 | by: api-sheriff-0-2-0/cleanup | rescoped: yes | evidence: integration-test-topology.adoc:613 and SVG:113 show variant instances (11) incl. plain-mgmt -- refutation already absorbed by the spec
