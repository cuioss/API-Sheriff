// ARCHIVED 2026-09-22 by the api-sheriff-0-2-0 orchestrator, from doc/plan/README.adoc.
//
// The index for the doc/plan/ directory, retired alongside its last two entries (01 and 09) by
// PLAN-V02-18-retire-doc-plan-survivors.md on explicit operator instruction, overriding
// PLAN-V02-03's landing ruling that had kept both files open. See the sibling archive files
// doc-plan-01-base-implementation.adoc and doc-plan-09-release-readiness.adoc for their content
// and provenance, and PLAN-V02-18 for the reasoning.
//
// This index already carried a stale reference of its own at archival time: its "Those landing
// records are local orchestrator artifacts..." paragraph points at
// `.plan/local/archived-orchestrators/api-sheriff-roadmap/landings/`, which no longer exists (the
// api-sheriff-roadmap epic's tree is gone from this checkout; verified absent 2026-09-22). Recorded
...
those records -- not a plan document -- are the authoritative account of what landed.

Those landing records are *local orchestrator artifacts, not tracked documentation*. They are
written as one `PLAN-*.md` file per plan under
`.plan/local/archived-orchestrators/api-sheriff-roadmap/landings/`, and because `.plan/` is
git-ignored they exist only in a working copy that ran the roadmap -- there is nothing here for a
reader to browse in the repository or on the published site.

The two plans below survive for different reasons: plan 01 is merged but predates the landing
corpus, so no record supersedes it; plan 09 is not yet delivered.
...
| All previously delivered work -- described by the link:../README.adoc[design documentation]
  and dated by the repository's own git history, both readable here. The landing records above
  are the orchestrator's audit trail of *how* that work landed, not a source a reader must open
...
* *Benchmarks follow the same rule for the hot path*: every plan that adds a runtime
  ...
  the plan that caused them. A plan that adds no hot-path behaviour is exempt -- there is
  nothing to measure; so is a plan whose subject *is* the benchmark harness, which repairs or
  replaces the measuring instrument rather than adding something to measure.
  link:09-release-readiness.adoc[Plan 09] consolidates baselines and regression thresholds; it
  does not create missing coverage.
* *Dependency approvals*: every new Maven dependency a plan needs is flagged inside the plan
  ...
  by `token-sheriff-bom`), never snapshots. The whole stack (validation + client modules) is
  already in the build, so a plan that consumes it wires an existing dependency rather than
  adding one -- and the approval gate applies only to what a plan genuinely introduces.
