...
5. Wait for CI and the automated reviewers — CodeRabbit and PR-Agent gate the merge; Sourcery is **optional** (`.plan/marshal.json` records `required_bots: coderabbit,cuioss-review-bot`, `optional_bots: sourcery`), so a Sourcery skip, absence or rate-limit never blocks (waits until checks complete): `gh pr checks --watch`
