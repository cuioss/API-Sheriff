...
* Port `8443` (HTTPS) — application endpoints (`/api/health`, `/api/info`)
* Port `9000` (plain HTTP) — management endpoints (`/q/health`, `/q/metrics`); bound to
  `127.0.0.1` (loopback only) so the unauthenticated management interface is not exposed
  on all host interfaces, matching the security posture documented in
