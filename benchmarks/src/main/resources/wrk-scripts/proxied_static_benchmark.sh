#!/bin/bash
# Benchmark runner for the proxied static backend (wrk -> gateway -> nginx).
# Measures proxy overhead: the gateway forwards /proxy/* to the fast static
# nginx backend, so the backend never saturates before the gateway does.
set -e
...
TARGET_URL="https://api-sheriff:8443/proxy/static"
...
docker compose run --rm wrk \
    ...
    --timeout "${WRK_TIMEOUT}" \
    --latency \
    -s /scripts/proxied_static_check.lua \
    "${TARGET_URL}"
