...
def resolve_property(repo: Path, name: str) -> tuple[str, str]:
    ...
    Both TokenSheriff and API-Sheriff hit this the moment they adopted ``cui-quarkus-parent``
    and stopped pinning ``version.quarkus`` themselves, so the text scan gets an effective-POM
    fallback.
