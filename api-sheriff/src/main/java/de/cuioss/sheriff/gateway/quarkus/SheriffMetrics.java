 ...
 *   <li>{@value #AUTH_BRANCH_TOTAL}{@code {route,branch}} — the authentication branch a
 *       {@code session_fallback} route selected per request, keyed by the {@link AuthBranch}
 *       label ({@code bearer} or {@code session}).</li>
 * </ul>
...
public class SheriffMetrics {
    ...
    /**
     * Counter of authentication-branch selections on {@code session_fallback} routes, keyed by route
     * and {@link AuthBranch} label ({@code bearer} / {@code session}).
     */
    public static final String AUTH_BRANCH_TOTAL = "sheriff_auth_branch_total";
    ...
    /**
     * Counts one authentication-branch selection on a {@code session_fallback} route against
     * {@link #AUTH_BRANCH_TOTAL}, tagged with the route id and the branch's lower-case
     * {@link AuthBranch#label() label}. Only the two branches a {@code session_fallback} route can
     * select are accepted, so the {@code branch} label never carries a value outside
     * {@code bearer} / {@code session}.
     *
     * @param route  the config-fixed route id of the {@code session_fallback} route
     * @param branch the selected branch, {@link AuthBranch#BEARER} or {@link AuthBranch#SESSION}
     * @throws IllegalArgumentException if {@code branch} is {@link AuthBranch#NONE}, which a
    ...
    public void recordAuthBranch(String route, AuthBranch branch) {
        ...
        registry.counter(AUTH_BRANCH_TOTAL, TAG_ROUTE, route, TAG_BRANCH, branch.label()).increment();
    }
